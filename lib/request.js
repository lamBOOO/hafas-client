...
const checkIfResponseIsOk = (_) => {
	...
	const getError = (_) => {
		...
		return {
			Error: HafasError,
			message: _.errTxt || 'unknown error',
			props: {},
		}
	}
	...
	if (svcRes.result && svcRes.result.resultCode !== 'OK') {
		const err = svcRes.result.externalError
		const errTxt = svcRes.result.internalError
		const {Error: HafasError, message, props} = getError({err, errTxt})
		throw new HafasError(message, err, {...errProps, ...props})
	}
}
